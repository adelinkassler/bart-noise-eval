...
format_practice_year_wide <- function(practice_year) {
  ...
  practice_year %>%
    ...
    mutate(Y_avg.post = (Y.y3 * n.patients.y3 + Y.y4 * n.patients.y4) /
           ...
           Y_outcome = Y_avg.post - Y_avg.pre) %>%
    select(-Y.y4, -Y.y3)
}
...
get_rhs_names <- function(x) {
  #expect_in(c("Y_outcome", 'id.practice'), names(x))
...
run_fit_and_pred <- function(data, dataset_name, .long) {
  ...
  saveRDS(bart.fit, glue("Save/fits/{version_suffix}/fit_{dataset_name}_BART.rds"))
  saveRDS(dart.fit, glue("Save/fits/{version_suffix}/fit_{dataset_name}_DART.rds"))
  
  pdep_post.bart <- pdep(bart.fit, data)
  pdep_post.dart <- pdep(dart.fit, data)
  saveRDS(pdep_post.bart, glue("Save/pdep_post/{version_suffix}/pdep_{dataset_name}_BART.rds"))
  saveRDS(pdep_post.dart, glue("Save/pdep_post/{version_suffix}/pdep_{dataset_name}_DART.rds"))
  
  bart.ests <- get_all_estimates(pdep_post.bart, data)
  dart.ests <- get_all_estimates(pdep_post.dart, data)
  write_csv(bart.ests, glue("Data/analyzed/{version_suffix}/{dataset_name}_BART.csv"))
  write_csv(dart.ests, glue("Data/analyzed/{version_suffix}/{dataset_name}_DART.csv"))
...
main.curia_merged <- function(data_path, .pscore = TRUE, .sdy = TRUE, .long = FALSE) {
  ...
  dataset_name <- paste(dataset_name, version_suffix, sep = '_')
